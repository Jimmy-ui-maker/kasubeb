...
export default function EditStudent({
  ...
  const [newAdmissionYear, setNewAdmissionYear] = useState(admissionYear);
  const [newAddress, setNewAddress] = useState(address);
  const [newContact, setNewContact] = useState(contact);
  const [newDob, setNewDob] = useState(dob);
  const [newSex, setNewSex] = useState(sex);
  const [newStateOfOrigin, setNewStateOfOrigin] = useState(stateOfOrigin);
  const [newLga, setNewLga] = useState(lga);
  const [newReligion, setNewReligion] = useState(religion);
  const [newSponsorName, setNewSponsorName] = useState(sponsorName);
  const [newSponsorAddress, setNewSponsorAddress] = useState(sponsorAddress);
  const [newSponsorContact, setNewSponsorContact] = useState(sponsorContact);
  const [newPhoto, setNewPhoto] = useState(imgUrl);

  const CLOUD_NAME = "drhagfkin";
  const UPLOAD_PRESET = "my_blog_project_sirjimmy";

  const [updatting, setIsupdatting] = useState(false);
  ...
  async function handleSubmit(e) {
    ...
    const response = await fetch(
      `http://localhost:3000/api/kadsubeb_api/${id}`,
      {
        method: "PUT",
        headers: {
    ...
    <>
      ...
      <section className="createForm min-vh-100 w-100 d-flex justify-content-center align-items-center ">
        <div className="container">
          ...
          <h2 className=" text-center badge ">EDIT STUDENT</h2>
              ...
              <div className="col-lg-3">
                <div className="mb-3 mt-1">
                  ...
                  <input
                    type="text"
                    className="form-control shadow-none mt-1"
                    onChange={(e) => setNewSponsorContact(e.target.value)}
                    value={newSponsorContact}
                  />
                </div>
              </div>
